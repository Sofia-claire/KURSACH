    ...
    if os.path.isdir(theme_dir):
        for f in os.listdir(theme_dir):
            if f.lower().endswith(".json"):
                ...
                themes[display_name] = os.path.join(theme_dir, f)
    return themes
...
def get_rus_appearance(val):
    return {"Light": "Светлая", "Dark": "Тёмная", "System": "Системная"}.get(val, "Тёмная")


def get_rus_theme(val):
    return {"blue": "Синяя", "green": "Зелёная", "dark-blue": "Тёмно-синяя"}.get(val, "Синяя")


def get_theme_colors():
    """Получает текущие цвета темы для синхронизации"""
    try:
        # Пытаемся достать цвета из движка CTk
        app = ctk.CTk()
        app.destroy()
        # Это хак, чтобы получить доступ к внутренней конфигурации темы
        theme_manager = ctk.ThemeManager.theme_manager
        current_theme = theme_manager._theme
        # Если тема кастомная, берем цвета оттуда, иначе дефолт
        return {
            "bg": current_theme.get("CTk", {}).get("fg_color", ["#EBEBEB", "#222428"])[
                0] if ctk.get_appearance_mode() == "Light" else
            current_theme.get("CTk", {}).get("fg_color", ["#EBEBEB", "#222428"])[1],
            "fg": current_theme.get("CTk", {}).get("fg_color_secondary", ["#D5D9DE", "#1A1D21"])[
                0] if ctk.get_appearance_mode() == "Light" else
            current_theme.get("CTk", {}).get("fg_color_secondary", ["#D5D9DE", "#1A1D21"])[1],
            "text": current_theme.get("CTkLabel", {}).get("text_color", ["gray14", "gray84"])[
                0] if ctk.get_appearance_mode() == "Light" else
            current_theme.get("CTkLabel", {}).get("text_color", ["gray14", "gray84"])[1]
        }
    except:
        mode = ctk.get_appearance_mode()
        return {"bg": "#EBEBEB" if mode == "Light" else "#222428",
                "fg": "#D5D9DE" if mode == "Light" else "#1A1D21",
                "text": "#1a1a1a" if mode == "Light" else "#e0e0e0"}
    ...
    for i in range(len(comb)):
        ...
        for j in range(i + 1, len(comb)):
            ...
            if fuzz.ratio(cn.lower(), str(comb[nc].iloc[j]).lower()) >= threshold:
                ...
                if dp:
                    grps[cn]['отделы'].add(dp)
                    grps[cn]['суммы_по_отделам'][dp] = grps[cn]['суммы_по_отделам'].get(dp, 0) + aj
...
class ConsolidationApp(ctk.CTk):
    def __init__(self):
        ...
        self.theme_map = load_available_themes()
        default_theme = "Синяя"
        if default_theme not in self.theme_map:
            default_theme = next(iter(self.theme_map), "blue")
    ...
    def update_window_icon(self):
        try:
            ...
            icon_path = os.path.join("ico", "dark.ico" if appearance == "Dark" else "light.ico")
    ...
    def setup_tray(self):
        try:
            appearance = ctk.get_appearance_mode()
            tray_icon_path = os.path.join("ico", "dark.ico" if appearance == "Dark" else "light.ico")
    ...
    def _update_ttk_style(self):
        """Синхронизирует стили ttk (таблицы, скроллы) с текущей темой customtkinter"""
        style = ttk.Style()
        mode = ctk.get_appearance_mode()
        if mode == "Dark":
            bg, fg, sel_bg, head_bg = "#2b2b2b", "#e0e0e0", "#005f99", "#3a3a3a"
        else:
            bg, fg, sel_bg, head_bg = "#f5f5f5", "#1a1a1a", "#0078d7", "#e0e0e0"
        ...
        style.configure("Treeview.Heading", background=head_bg, foreground=fg, font=('TkDefaultFont', 10, 'bold'))
        style.map("Treeview.Heading", background=[('active', 'gray50')])

        style.configure("Vertical.TScrollbar", background=bg, troughcolor=bg, borderwidth=0, arrowcolor=fg)
        style.configure("Horizontal.TScrollbar", background=bg, troughcolor=bg, borderwidth=0, arrowcolor=fg)
    ...
    def create_navigation_frame(self):
        ...
        # --- ЛОГИКА ЗАГРУЗКИ ИКОНКИ МЕНЮ ---
        appearance = ctk.get_appearance_mode()
        icon_name = "dark.ico" if appearance == "Dark" else "light.ico"
        icon_path = os.path.join("ico", icon_name)

        img = None
        if os.path.exists(icon_path):
            try:
                img = ctk.CTkImage(light_image=Image.open(icon_path), size=(128, 128))
            except Exception as e:
                print(f"⚠️ Ошибка загрузки иконки меню: {e}")

        # Если картинка загрузилась - текст пустой, иначе показываем текст "МЕНЮ"
        text_content = "" if img else " 📑 МЕНЮ "
        ctk.CTkLabel(self.nav, text=text_content, image=img, font=ctk.CTkFont(size=18, weight="bold")).grid(row=0,
                                                                                                            column=0,
                                                                                                            padx=20,
                                                                                                            pady=20)
        ...
        ctk.CTkButton(self.nav, text="🚪 Выход", command=self.on_closing, fg_color="#d32f2f",
                      hover_color="#b71c1c").grid(row=row_idx + 1, column=0, pady=10, padx=20, sticky="ew")
    ...
    def create_home_frame(self):
        f = ctk.CTkFrame(self, corner_radius=0)  # Убран transparent
        ...
        ctk.CTkLabel(f, text=info, justify="left", wraplength=850, font=ctk.CTkFont(size=14)).grid(row=1, column=0,
                                                                                                   pady=30, padx=50,
                                                                                                   sticky="w")
        separator = ctk.CTkFrame(f, height=2)  # Убран fg_color
    ...
    def create_settings_frame(self):
        ...
        self.theme_combo = ctk.CTkComboBox(box, values=list(self.theme_map.keys()), variable=self.theme_var,
    ...
    def create_admin_frame(self):
        ...
        ctk.CTkLabel(f, text="Активные ключи: ", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=40,
                                                                                       pady=(20, 5))
    ...
    def create_users_frame(self):
        ...
        ctk.CTkButton(f, text="🗑️ Удалить выбранного", command=self.delete_user, fg_color="#d32f2f",
                      hover_color="#b71c1c").pack(pady=5)
        ctk.CTkButton(f, text="🔄 Обновить список", command=self.load_users_ui).pack(pady=5)
        return f

    def rebuild_ui(self):
        current = "consolidator"
        frames_map = [("home", self.home_frame), ("consolidator", self.con_frame), ("settings", self.set_frame),
                      ("history", self.hist_frame)]
    ...
    def change_theme(self, choice):
        theme_value = self.theme_map.get(choice, "blue")
        ctk.set_default_color_theme(theme_value)
        # Микрозадержка гарантирует, что customtkinter применит тему ДО пересоздания виджетов
        self.after(10, self.rebuild_ui)
    ...
    def save_settings_to_db(self):
        ...
        for e, t, _ in self.cat_widgets:
            try:
                if not e.winfo_exists() or not t.winfo_exists(): continue
