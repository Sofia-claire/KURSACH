...
def load_config():
    defaults = {"threshold": 85, "appearance": "Dark", "theme": "blue", "categories": DEFAULT_CATEGORIES}
...
# ==================== 1. ТЕСТОВЫЕ ДАННЫЕ ====================
def create_realistic_files(folder_path='тестовые_отчёты_расширенные', categories=None):
    ...
    for dept in departments:
        ...
        for _ in range(num_transactions):
            ...
            name_variant = random.choice(categories[category])
...
class ConsolidationApp(ctk.CTk):
    def __init__(self):
        ...
        self.expense_categories = self.config.get("categories", DEFAULT_CATEGORIES)
        self.cat_widgets = []  # Хранит ссылки на виджеты категорий: (name_entry, kw_entry, row_frame)
    ...
    def create_navigation_frame(self):
        ...
        self.appearance_mode_menu = ctk.CTkOptionMenu(self.navigation_frame, variable=self.appearance_var,
    ...
    def create_settings_frame(self):
        frame = ctk.CTkFrame(self, corner_radius=0, fg_color="transparent")
        ctk.CTkLabel(frame, text="⚙️ Настройки системы", font=ctk.CTkFont(size=20, weight="bold")).pack(pady=20)

        box = ctk.CTkFrame(frame, width=600)
        ...
        # 🔽 РЕДАКТОР КАТЕГОРИЙ 🔽
        ctk.CTkLabel(box, text="📂 Категории расходов (для тестовых данных):",
                     font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=30, pady=(20, 5))

        self.cat_scroll = ctk.CTkScrollableFrame(box, height=160)
        self.cat_scroll.pack(fill="x", padx=30, pady=5)

        ctk.CTkButton(box, text="➕ Добавить категорию", command=lambda: self.add_category_row_ui(), width=180).pack(
            pady=(5, 10))

        # Заполняем существующие категории
        for name, kws in self.expense_categories.items():
            self.add_category_row_ui(name, ", ".join(kws))

        ctk.CTkLabel(box, text="💡 Изменения сохраняются при закрытии программы.", text_color="gray",
                     wraplength=500).pack(pady=(10, 20))
        return frame
    ...
    def add_category_row_ui(self, name="", keywords=""):
        row = ctk.CTkFrame(self.cat_scroll)
        row.pack(fill="x", pady=2)

        # Поле названия категории (фиксированная ширина)
        ent_name = ctk.CTkEntry(row, placeholder_text="Название категории", width=150)
        ent_name.pack(side="left", padx=2)
        ent_name.insert(0, name)

        # ✅ Поле ключевых слов (растягивается и переносит текст)
        txt_kw = ctk.CTkTextbox(row, height=40, width=300)
        txt_kw.pack(side="left", fill="x", expand=True, padx=2)
        txt_kw.configure(wrap="word")  # Включаем перенос строк
        txt_kw.insert("1.0", keywords)  # Вставляем существующие данные

        # Кнопка удаления
        btn_del = ctk.CTkButton(row, text="🗑️", width=35, command=lambda: self.remove_category_row_ui(row))
        btn_del.pack(side="left", padx=2)

        # Сохраняем ссылку на текстовое поле вместо Entry
        self.cat_widgets.append((ent_name, txt_kw, row))

    def remove_category_row_ui(self, row_frame):
        row_frame.destroy()
        self.cat_widgets = [(n, k, r) for n, k, r in self.cat_widgets if r != row_frame]

    def save_categories_from_ui(self):
        cats = {}
        for name_ent, kw_txt, _ in self.cat_widgets:
            n = name_ent.get().strip().lower().replace(" ", "_")

            # ✅ Считываем текст из Textbox (убираем последний символ новой строки)
            k_raw = kw_txt.get("1.0", "end-1c")
            k = [kw.strip() for kw in k_raw.split(",") if kw.strip()]

            if n and k:
                cats[n] = k
        self.expense_categories = cats
    ...
    def on_closing(self):
        ...
        self.save_categories_from_ui()  # ✅ Сохраняем категории
        save_config(self.config)
        self.destroy()
    ...
    def create_test_data(self):
        try:
            ...
            # ✅ Передаём актуальные категории
            files = create_realistic_files(folder_path, self.expense_categories)
